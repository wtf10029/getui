...
class HttpRequest
{
    ...
    public function send()
    {
        try {
            ...
            $this->data['sign'] = hash("sha256",
                "{$this->config->getAppKey()}{$this->data['timestamp']}{$this->config->getMasterSecret()}");
            if ($this->authorization instanceof Authorization && method_exists($this->authorization,
                    'getTokenAsString')) {
